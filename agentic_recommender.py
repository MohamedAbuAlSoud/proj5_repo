...
def get_user_skills_tool(user_id):
    """Retrieves stored user skills from PostgreSQL."""
    try:
...
def search_courses_tool():
    """Fetches all courses with complete metadata from database."""
    try:
        with db.engine.connect() as conn:
...
def generate_learning_path_tool(recommended_courses):
    """Multi-Agent Learning Path Planner (Sequences courses by difficulty)."""
...
def get_user_feedback_score_adjustment(course_id):
    """Calculates score boost/penalty based on Recommendation Feedback Loop."""
    try:
        with db.engine.connect() as conn:
